...
// {
//     pub fn tensor_product<const P: usize>(v: [V<M, F>; P], w: [V<N, F>; P])
// -> Tensor<M, N, F> {         let mut tensor = Tensor::default();
...
//                     self.coefficients.0[i].0[j] +
// other.coefficients.0[i].0[j];             }
//         }
//         tensor
//     }
// }
...
//         for i in 0..M {
//             for j in 0..N {
//                 tensor.coefficients.0[i].0[j] = self.coefficients.0[i].0[j] *
// scalar;             }
//         }
//         tensor
//     }
...
//     /// Here, for each choice of `w`, we get a distinct linear functional on
// `V`     /// that utilizes the tensor product.
//     #[allow(non_snake_case)]
//     pub fn get_functional_on_V(&self, w: V<N, F>) -> impl Fn(V<M, F>) -> F +
// '_ {         move |v| self.bilinear_map(v, w)
//     }

//     /// Here, for each choice of `v`, we get a distinct linear functional on
// `W`     /// that utilizes the tensor product.
//     #[allow(non_snake_case)]
//     pub fn get_functional_on_W(&self, v: V<M, F>) -> impl Fn(V<N, F>) -> F +
// '_ {         move |w| self.bilinear_map(v, w)
//     }
...
// /// In other words, we can multiply M x N matrices with N x P matrices to get
// an /// M x P matrix.
// impl<const M: usize, const N: usize, const P: usize, F> Mul<Tensor<N, P, F>>
// for Tensor<M, N, F> where
//     [(); M * N]:,
//     [(); N * P]:,
