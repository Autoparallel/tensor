use core::{marker::PhantomData, ops::Div};

use super::*;

pub trait Module:
    Add<Output = Self> + Neg<Output = Self> + Mul<Self::Ring, Output = Self> + Copy
{
    type Ring: Add + Neg + Mul + Default + ConstDefault + Copy;
}

pub trait VectorSpace: Module
where
    Self::Ring: Div,
{
}

#[derive(Copy, Clone, Debug)]
pub struct Vector<const M: usize, F>(pub [F; M]);

impl<const M: usize, F> ConstDefault for Vector<M, F>
where
    F: ConstDefault + Copy,
{
    const DEFAULT: Self = Self([F::DEFAULT; M]);
}

impl<const M: usize, F: ~const core::ops::Add<Output = F> + ConstDefault + Copy> const Add
    for Vector<M, F>
{
    type Output = Self;
    fn add(self, other: Self) -> Self::Output {
        let mut sum = Self::DEFAULT;
        let mut i = 0;
        while i < M {
            sum.0[i] = self.0[i] + other.0[i];
            i += 1;
        }
        sum
    }
}

impl<const M: usize, F: Neg<Output = F> + ConstDefault + Copy> Neg for Vector<M, F> {
    type Output = Self;
    fn neg(self) -> Self::Output {
        let mut neg = Self::DEFAULT;
        for i in 0..M {
            neg.0[i] = -self.0[i];
        }
        neg
    }
}

impl<const M: usize, F: Mul<Output = F> + ConstDefault + Copy> Mul<F> for Vector<M, F> {
    type Output = Self;
    fn mul(self, scalar: F) -> Self::Output {
        let mut scalar_multiple = Self::DEFAULT;
        let mut i = 0;
        while i < M {
            scalar_multiple.0[i] = scalar * self.0[i];
            i += 1;
        }
        scalar_multiple
    }
}

impl<
        const M: usize,
        F: Add<Output = F> + Neg<Output = F> + Mul<Output = F> + Default + ConstDefault + Copy,
    > Module for Vector<M, F>
{
    type Ring = F;
}

impl<
        const M: usize,
        F: Add<Output = F> + Neg<Output = F> + Mul<Output = F> + Div + Default + ConstDefault + Copy,
    > VectorSpace for Vector<M, F>
{
}

#[derive(Clone, Copy, Default)]
pub struct TrivialModule<R> {
    pub(crate) _r: PhantomData<R>,
}

impl<R: Add + Neg + Mul + Default + ConstDefault + Copy> Module for TrivialModule<R> {
    type Ring = R;
}

impl<R: Copy + Default> Add for TrivialModule<R> {
    type Output = Self;
    fn add(self, _: Self) -> Self::Output {
        Self { _r: PhantomData }
    }
}

impl<R: Copy + Default> Neg for TrivialModule<R> {
    type Output = Self;
    fn neg(self) -> Self::Output {
        Self { _r: PhantomData }
    }
}

impl<R: Copy + Default> Mul<R> for TrivialModule<R> {
    type Output = Self;
    fn mul(self, _: R) -> Self::Output {
        Self { _r: PhantomData }
    }
}
